...
import { Upload, Download } from "lucide-react"; // Removed unused icons
...
import { ProductData } from "./profit-margin-calculator/types"; // Import ProductData only
import { calculateProfitMetrics } from "@/lib/profit-margin-utils"; // Import calculation logic
...
export default function ProfitMarginCalculator() {
  ...
  const handleCalculateResults = useCallback(
    (data: ProductData[]) => {
      ...
      const calculated = calculateProfitMetrics(data); // Use the modularized function
  ...
  const handleFileUpload = useCallback(
      ...
      Papa.parse<ProductData>(file, {
        ...
        complete: (result) => {
          try {
            ...
            handleCalculateResults(processedData);
          } catch (err) {
    ...
    [handleCalculateResults, toast, setIsLoading, setError],
  );
  ...
  const handleManualSubmit = useCallback(
    (e: React.FormEvent) => {
      ...
      handleCalculateResults([newProduct]);
      setManualProduct({
        product: "",
        cost: 0,
        price: 0,
    ...
    [manualProduct, handleCalculateResults, toast, setError, setManualProduct],
  );
  ...
  return (
    <ToolContainer
      ...
      description="Calculate and analyze your product's profitability."
      isLoading={isLoading}
      error={error}
    >
      {/* Input Section */}
